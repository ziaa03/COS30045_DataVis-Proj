  ...
  useEffect(() => {
    ...
    d3.csv('/datasets/oecd_pm25_exposure.csv').then(function(data_oecd) {
      ...
      // d3.csv('/datasets/cardiovascular_death_rate.csv').then(function(data_sub) {
        ...
        const country_oecd = data_oecd.find(d => d['Reference area'] === 'Australia');
        const country_sub = data_sub.find(d => d['Country'] === 'Australia');
        ...
        var xScale = d3.scaleLinear() // Use scaleTime for time-based data like years
            .domain([
                d3.min(dataset_oecd, function(d) { return d.year; }), // Convert to Date object
                d3.max(dataset_oecd, function(d) { return d.year; })  // Convert to Date object
            ])
        ...
        var yScale = d3.scaleLinear()
                        .domain([
                          ...
                          0,
                          d3.max(dataset_oecd, function(d) {return d.value; })
                        ])
        ...
        var yScale_right = d3.scaleLinear()
                        .domain([
                          ...
                          0,
                          d3.max(dataset_sub, function(d) {return d.sub_value; })
                        ])
                        .range([h - padding, 10 + padding]);
